...
import classes from "./flowgauge.module.css";

class meterGauge extends Component {
    componentDidMount() {
        this.drawMeter();
    }
    ...
    drawMeter = () => {
        ...
        let gauge = d3.select("#" + this.props.gaugeID).append("svg").attr("id", "gauge" + this.props.gaugeID).attr("class", classes.gauge);
    ...
    render() {
        return <div id={this.props.gaugeID} className={classes.gaugeBlock}>
            <div className={classes.gaugeDesc}>{this.props.gaugeDesc}</div>
            <div className={classes.gaugeInfo}>{this.props.gaugeInfo}</div>
        </div>
    }
}
